...
export const EditorView: React.FC = () => {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl space-y-4 px-3 py-4 pb-28 text-left animate-fade-in sm:px-4">
      {/* Top Header Bar */}
      ...
      <div className="sticky bottom-0 z-20 -mx-3 border-t border-slate-800/90 bg-[#0b131e]/95 px-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-3 backdrop-blur sm:-mx-4 sm:px-4">
        <button
          onClick={() => setIsPublishModalOpen(true)}
          disabled={blocks.length === 0 || !activeChannel}
          title={!activeChannel ? 'Select a channel before publishing' : undefined}
          className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#0088cc] px-6 py-4 text-sm font-extrabold uppercase tracking-wider text-white shadow-xl shadow-sky-500/25 transition hover:bg-[#0077b5] active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-50"
        >
