...
export const ChannelCard: React.FC<Props> = ({
  ...
  const subscriberCount = typeof channel.memberCount === 'number'
    ? new Intl.NumberFormat('en', { notation: 'compact', maximumFractionDigits: 1 }).format(channel.memberCount)
    : null;
        ...
        <div className="text-left overflow-hidden">
          ...
          <div className="flex items-center space-x-2 text-xs text-slate-400 mt-0.5">
            ...
            <div className="flex items-center space-x-1 text-slate-300">
              ...
              <span>{subscriberCount ? `${subscriberCount} subscribers` : 'Count unavailable'}</span>
            </div>
          </div>
        </div>
      </button>
      ...
      <div className="flex shrink-0 items-center gap-1">
        {onDelete && (
          <button
            type="button"
            onClick={onDelete}
            className="inline-flex items-center gap-1.5 rounded-lg border border-rose-500/25 bg-rose-500/10 px-2 py-1.5 text-xs font-semibold text-rose-300 transition hover:bg-rose-500/20 hover:text-rose-200"
