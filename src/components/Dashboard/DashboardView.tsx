...
export const DashboardView: React.FC<Props> = ({ onOpenRoundVideo }) => {
  ...
  const [channelToDelete, setChannelToDelete] = useState<typeof channels[number] | null>(null);
    ...
    <div className="mx-auto w-full max-w-2xl space-y-5 px-3 py-4 animate-fade-in sm:space-y-6 sm:px-4 sm:py-6">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            {channels.length > 0 && (
              <button
                ...
                className={`rounded-lg px-2.5 py-1.5 text-xs font-semibold transition ${isManagingChannels ? 'bg-sky-500 text-white' : 'bg-slate-800 text-sky-400 hover:bg-slate-700 hover:text-sky-300'}`}
              >
                {isManagingChannels ? 'Done' : 'Manage channels'}
              </button>
            )}
            <button
      ...
      {channelToDelete && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="presentation">
          <section role="dialog" aria-modal="true" aria-labelledby="remove-channel-title" className="w-full max-w-sm rounded-3xl border border-slate-700 bg-[#17212b] p-5 shadow-2xl">
            <h3 id="remove-channel-title" className="text-base font-extrabold text-white">Remove channel?</h3>
            <p className="mt-2 text-sm leading-relaxed text-slate-400">
              <span className="font-semibold text-slate-200">{channelToDelete.title}</span> will be removed from this posting studio. Your Telegram channel and existing posts will not be deleted.
            </p>
            <div className="mt-5 flex justify-end gap-2">
              <button type="button" onClick={() => setChannelToDelete(null)} className="rounded-xl px-3.5 py-2 text-sm font-semibold text-slate-300 transition hover:bg-slate-800">Cancel</button>
              <button
                type="button"
                onClick={() => {
                ...
                className="rounded-xl bg-rose-500 px-3.5 py-2 text-sm font-bold text-white transition hover:bg-rose-400"
              >
                Remove channel
              </button>
            </div>
