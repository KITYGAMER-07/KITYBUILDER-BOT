...
export const TelegramPreview: React.FC = () => {
  ...
  const hasContent = blocks.some((block) =>
    Boolean(block.content?.trim() || block.metadata?.mediaUrl || block.metadata?.mediaUrls?.length || block.metadata?.mapLocation),
  );
            ...
            {blocks.map((block) => {
              switch (block.type) {
                case 'photo': {
                  ...
                  return (
                    <figure key={block.id} className="overflow-hidden rounded-xl border border-slate-700/70 bg-black/30">
                      <img src={block.metadata.mediaUrl} alt={block.content || 'Post media'} className="max-h-80 w-full object-cover" />
                ...
                case 'video': {
                  ...
                  return (
                    <figure key={block.id} className="overflow-hidden rounded-xl border border-slate-700/70 bg-black/30">
                      <video src={block.metadata.mediaUrl} controls playsInline preload="metadata" className="max-h-80 w-full bg-black object-contain" />
                      {block.content && <figcaption className="px-3 py-2 text-xs text-slate-200">{block.content}</figcaption>}
                    </figure>
                  );
                }
                ...
                case 'animation': {
                  ...
                  return isRoundVideo ? (
                    ...
                    <figure key={block.id} className="overflow-hidden rounded-xl border border-slate-700/70 bg-black/30">
                      <img src={block.metadata.mediaUrl} alt={block.content || 'Animation'} className="max-h-80 w-full object-cover" />
                      {block.content && <figcaption className="px-3 py-2 text-xs text-slate-200">{block.content}</figcaption>}
                    </figure>
                  );
                }
                  ...
                  return (
                    <div
                      ...
                      className={`font-bold text-white ${
                        level === 1 ? 'text-base underline decoration-sky-500/70 underline-offset-4' : 'text-sm'
                      }`}
                    >
                ...
                case 'fold': {
                  const isRevealed = revealedSpoilers[block.id];
                  return (
                    <button
                      type="button"
                      key={block.id}
                      onClick={() => toggleSpoiler(block.id)}
                ...
                case 'table': {
                  return (
                    <pre key={block.id} className="overflow-x-auto rounded-xl border border-slate-700/80 bg-[#0f172a] p-3 text-xs text-slate-200">
